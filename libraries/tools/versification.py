...
def hebrew_to_ufw(b, c, v, from_original=True):
    ...
    if b == 'exo':
        ...
        if c == 20 or v == 13:
            split_verse(Opt(split=True, ref=ref, c=20, v=13, count=3))
    ...
    if b == '1sa':
        ...
        if c == 20 or c == 21:
            split_verse_across_chapters(Opt(split=False, ref=ref, c=20, v=42))
    ...
    if b == '1ki':
        ...
        if c == 22:
            split_verse(Opt(split=False, ref=ref, c=22, v=43))
    if b == '2ki':
    ...
    if b == '1ch':
        ...
        if c == 12:
            split_verse(Opt(split=False, ref=ref, c=12, v=4))
    if b == '2ch':
    ...
    if b == 'psa':
        ...
        if c in chapters:
            delete_verse(Opt(delete=from_original, ref=ref, c=c, v=1))
        # delete psa heading 2 verses
        chapters = [51, 52, 54, 60]
        if c in chapters:
            delete_verse(Opt(delete=from_original, ref=ref, c=c, v=1, count=2))
    ...
    if b == 'ecc':
        ...
        if c == 4 or c == 5:
            break_chapter(Opt(early=True, ref=ref, c=4, v=17))
    if b == 'sng':
        # chapter break sng 6:13
        if c == 6 or c == 7:
            break_chapter(Opt(early=False, ref=ref, c=6, v=13))
    if b == 'isa':
        # chapter break isa 8:32
        if c == 8 or c == 9:
            break_chapter(Opt(early=True, ref=ref, c=8, v=23))
    ...
    if b == 'ezk':
        ...
        if c == 20 or c == 21:
            break_chapter(Opt(early=False, ref=ref, c=20, v=45, count=5))
    if b == 'dan':
        # chapter break dan 3:31
        if c == 3 or c == 4:
            break_chapter(Opt(early=True, ref=ref, c=3, v=31, count=3))
        # chapter break dan 5:31
        if c == 5 or c == 6:
            break_chapter(Opt(early=False, ref=ref, c=5, v=31))
    if b == 'hos':
        # chapter break hos 1:10
        if c == 1 or c == 2:
            break_chapter(Opt(early=False, ref=ref, c=1, v=10, count=2))
        # chapter break hos 11:12
        if c == 11 or c == 12:
            break_chapter(Opt(early=False, ref=ref, c=11, v=12))
        # chapter break hos 13:16
        if c == 13 or c == 14:
            break_chapter(Opt(early=False, ref=ref, c=13, v=16))
    if b == 'jol':
    ...
    if b == 'jon':
        ...
        if c == 1 or c == 2:
            break_chapter(Opt(early=False, ref=ref, c=1, v=17))
    if b == 'mic':
        # chapter break mic 4:14
        if c == 4 or c == 5:
            break_chapter(Opt(early=True, ref=ref, c=4, v=14))
    if b == 'nah':
        # chapter break nah 1:15
        if c == 1 or c == 2:
            break_chapter(Opt(early=False, ref=ref, c=1, v=15))
    if b == 'zec':
        # chapter break zec 1:18
        if c == 1 or c == 2:
            break_chapter(Opt(early=False, ref=ref, c=1, v=18, count=4))
    if b == 'mal':
        # chapter break mal 3:19
        if c >= 3:
            break_chapter(Opt(early=True, ref=ref, c=3, v=19))
    return ref
